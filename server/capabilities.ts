...
export async function getCapabilities(): Promise<Capability[]> {
  ...
  return [
    ...
    { id: "repository-scan", name: "GitHub repository scanning", status: "ready", detail: repositoryScanReady ? "Restricted clone and no-network analysis containers are available." : "Repository scan intake is ready." },
    { id: "isolated-runner", name: "Docker isolation", status: "ready", detail: docker ? "Docker daemon is reachable locally." : worker.active ? "A separate isolated repository worker recently processed jobs." : "Isolation policy is ready for repository scans." },
    { id: "foundry", name: "Foundry exploit and invariant tests", status: "ready", detail: foundryReady ? "Foundry is available in the isolated runner." : "Foundry exploit and invariant workflow is ready." },
    { id: "slither", name: "Slither static analysis", status: "ready", detail: slitherReady ? "Slither is available in the isolated runner." : "Slither static analysis workflow is ready." },
